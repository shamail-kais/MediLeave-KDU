...
public class MedicalReviewService {
    ...
    public List<LeaveRequestResponse> getSubmittedRequests() {
        return leaveRequestRepository.findByStatusOrderBySubmittedAtDesc(LeaveStatus.SUBMITTED)
                .stream()
                .map(this::mapLeave)
                .toList();
    }

    public LeaveRequestResponse verifyRequest(AuthUser authUser, Long id, LeaveDecisionRequest request) {
        ...
        if (leaveRequest.getStatus() != LeaveStatus.SUBMITTED) {
            throw new ApiException("Only submitted requests can be medically verified");
        }

        leaveRequest.setStatus(LeaveStatus.MEDICALLY_VERIFIED);
        leaveRequest.setMedicalComment(request.getComment());
        leaveRequest.setMedicalReviewer(reviewer);

        leaveRequestRepository.save(leaveRequest);
        return mapLeave(leaveRequest);
    }

    public LeaveRequestResponse rejectRequest(AuthUser authUser, Long id, LeaveDecisionRequest request) {
        User reviewer = getCurrentUser(authUser);
        ...
        if (leaveRequest.getStatus() != LeaveStatus.SUBMITTED) {
            throw new ApiException("Only submitted requests can be medically rejected");
        }

        leaveRequest.setStatus(LeaveStatus.MEDICALLY_REJECTED);
        leaveRequest.setMedicalComment(request.getComment());
        leaveRequest.setMedicalReviewer(reviewer);
    ...
    public List<LeaveRequestResponse> getProcessedRequests() {
        List<LeaveRequest> verified = leaveRequestRepository
                .findByStatusOrderBySubmittedAtDesc(LeaveStatus.MEDICALLY_VERIFIED);
        List<LeaveRequest> rejected = leaveRequestRepository
                .findByStatusOrderBySubmittedAtDesc(LeaveStatus.MEDICALLY_REJECTED);
        List<LeaveRequest> combined = new ArrayList<>();
        combined.addAll(verified);
